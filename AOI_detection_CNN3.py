...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun  7 08:24:11 2018
...
np.random.seed(3366)
index_OK=np.arange(3960)
np.random.shuffle(index_OK)
np.random.seed(6633)
index_NG=np.arange(360)
np.random.shuffle(index_NG)
img_size1=64
img_size2=32
...
x_test2=np.zeros((numOfTest2,img_size1*img_size2))
y_test2=np.zeros(numOfTest2,dtype='int8')
for i in range(len(classes)):
    dir_input='AOIDetection_da/'+classes[i]+'/*.png'
    coll=io.ImageCollection(dir_input)
    if i==0:
        index_array=index_OK
    else:
        index_array=index_NG
    for j in range(numOfTrain+numOfTest):
#    for j in range(1):
        index=index_array[j]
        img=cv2.imread(coll.files[index],0)
...
dir_input='AOIDetection_da/'+classes[0]+'/*.png'
coll=io.ImageCollection(dir_input)            
for i in range(numOfTest2):
    k=i+numOfTrain+numOfTest
    index=index_OK[k]
    img=cv2.imread(coll.files[index],0)
    img_flatted=img.flatten()
    x_test2[i,:]=img_flatted
    y_test2[i]=0
...
x_test_2=np.zeros((numOfTest2,img_size1*img_size2))
y_test_2oneHot=np.zeros((numOfTest2,len(classes)))
...
for i in range(numOfTest2):
    x_test_2[i,:]=x_test2[i,:]
    y_test_2oneHot[i,0]=1
...
with tf.Session() as sess:
            ...
            if i < np.int32(numOfTest*n_classes/batch_size):
                accuracy_test=sess.run(Accuracy_tf,
                                       feed_dict ={Xs:x_test_shuffled[i_start:i_end,:],
                                        ys:y_test_oneHot[i_start:i_end,:],keep_prob:1})
            losses.append(loss)
    ...
    preds2=[]
    for i in range(np.int32(numOfTest2/batch_size)):
        i_start=i*batch_size
        i_end=(i+1)*batch_size
        pred2=sess.run(prediction,{Xs:x_test_2[i_start:i_end],
                                  ys:y_test_2oneHot[i_start:i_end],keep_prob:0.7})
        tmp2=np.argmax(pred2,axis=1)
        preds2.extend(tmp2)
        
    delta2=preds2-np.argmax(y_test_2oneHot,axis=1)
    delta_2=np.where(delta2==0)
    accuracy2=np.size(delta_2[0])/(numOfTest2)
    print ("___________________________")
    print (accuracy2)
